      ...
      {result !== null && (
        <div ref={pdfRef} className="mt-6 p-6 bg-white border-2 rounded-sm break-inside-avoid">
            ...
            {!result.isCompliant && result.eplLimit && (
              <div className="mt-4 pt-3 border-t border-red-300 bg-white/50 p-3 rounded-sm break-inside-avoid">
                ...
                <p className="font-mono font-bold text-lg text-[#0B3B5C]">
                  {result.eplLimit} kW <span className="text-xs font-normal text-gray-600">(from {meMcr} kW MCR)</span>
                </p>
              </div>
            )}
          </div>
          <div className="bg-gray-50 p-3 rounded-sm border text-[10px] text-gray-600 leading-relaxed">
            ...
            <p>
              Attained EEXI calculated per MEPC.338(76): (SFC × CF × MCR) / (fi × fc × DWT × Vref). PTO reduction applied as: f_eff × P_PTO × SFC_ME × CF. EPL estimated using cubic relationship: EPL = MCR × (Required/Attained)^(1/3) per MEPC.338(76). Weather corrections (fw) assume standard sea trials. Excludes C_FV hull correction factors. This document is NOT an IEE Certificate or Class-approved EEXI technical file.
            </p>
          </div>
        </div>
      )}
