...
export function EexiCalculator() {
  ...
  const handleCalculate = () => {
    ...
    const attainedEexi = totalEmissions / (typeData.fi * typeData.fc * dwtNum * vrefNum);
    ...
    if (!finalResult.isCompliant) trackComplianceFail("eexi");
    window.dispatchEvent(new CustomEvent("tool_compliance_update", { detail: { status: finalResult.isCompliant ? "compliant" : "non-compliant" } }));
  };
  ...
  return (
    <div className="bg-white border border-border/40 rounded-sm p-6 md:p-8 shadow-sm">
        ...
        <div className="p-4 bg-neutral-50 rounded-sm border border-border/20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select value={meFuel} onChange={e => setMeFuel(e.target.value)} className="w-full p-2 border rounded-sm bg-white text-base focus:border-primary outline-none">
                {FUEL_TYPES.map(f => <option key={f.value} value={f.value}>{f.label}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-xs font-medium text-muted-foreground mb-1">SFC (g/kWh)</label>
              <input type="number" value={meSfc} onChange={e => setMeSfc(e.target.value)} placeholder={DEFAULT_SFC_ME.toString()} className="w-full p-2 border rounded-sm text-base focus:border-primary outline-none" />
            </div>
            <div>
              <label className="block text-xs font-medium text-muted-foreground mb-1">Vref (Knots)</label>
              <input type="number" step="0.1" value={vref} onChange={e => setVref(e.target.value)} placeholder="14.5" className="w-full p-2 border rounded-sm text-base focus:border-primary outline-none" />
            </div>
          </div>
        </div>

        <div className="p-4 bg-neutral-50 rounded-sm border border-border/20">
          <h3 className="text-sm font-bold text-[#0B3B5C] mb-3 uppercase tracking-wider">Auxiliary & Power Take-Off (PTO)</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-medium text-muted-foreground mb-1">Total Auxiliary Power — PAE (kW)</label>
                <input type="number" value={auxPower} onChange={e => setAuxPower(e.target.value)} placeholder="500" className="w-full p-2 border rounded-sm text-base focus:border-primary outline-none" />
              </div>
              <div>
                <label className="block text-xs font-medium text-muted-foreground mb-1">Aux SFC (g/kWh)</label>
                <input type="number" value={auxSfc} onChange={e => setAuxSfc(e.target.value)} placeholder={DEFAULT_SFC_AUX.toString()} className="w-full p-2 border rounded-sm text-base focus:border-primary outline-none" />
              </div>
            </div>
              ...
              {hasPto && (
                <>
                  <div>
                    ...
                    <input type="number" value={ptoPower} onChange={e => setPtoPower(e.target.value)} placeholder="1000" className="w-full p-2 border rounded-sm text-base focus:border-primary outline-none" />
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-muted-foreground mb-1">PTO Efficiency Factor (f_eff)</label>
                    <input type="number" step="0.1" value={ptoEff} onChange={e => setPtoEff(e.target.value)} placeholder="1.0" className="w-full p-2 border rounded-sm text-base focus:border-primary outline-none" />
                  </div>
                </>
              )}
            </div>
      ...
      <button onClick={handleCalculate} className="w-full py-3 bg-primary text-white font-medium rounded-sm hover:bg-primary/90 transition mt-6">Calculate Attained EEXI</button>
        ...
        <div ref={pdfRef} className="mt-6 p-6 bg-white border-2 rounded-sm no-break">
          ...
          <div className={`p-4 border-2 rounded-sm mb-4 ${result.isCompliant ? "border-green-500 bg-green-50" : "border-red-500 bg-red-50"}`}>
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div><span className="text-xs text-gray-600">Attained EEXI:</span><p className="font-bold text-xl text-[#0B3B5C]">{result.attained}</p></div>
              <div><span className="text-xs text-gray-600">Required EEXI ({targetYear}):</span><p className="font-bold text-xl text-[#0B3B5C]">{result.required}</p></div>
            </div>
            {!result.isCompliant && result.eplLimit && (
              <div className="mt-4 pt-3 border-t border-red-300 bg-white/50 p-3 rounded-sm no-break">
                ...
                <p className="font-mono font-bold text-lg text-[#0B3B5C]">{result.eplLimit} kW <span className="text-xs font-normal text-gray-600">(from {meMcr} kW MCR)</span></p>
              </div>
            )}
          </div>
          <div className="bg-gray-50 p-3 rounded-sm border text-[10px] text-gray-600 leading-relaxed">
            ...
            <p>Attained EEXI calculated per MEPC.338(76): (SFC × CF × MCR) / (fi × fc × DWT × Vref). PTO reduction applied as: f_eff × P_PTO × SFC_ME × CF. EPL estimated using cubic relationship: EPL = MCR × (Required/Attained)^(1/3) per MEPC.338(76). Weather corrections (fw) assume standard sea trials. Excludes C_FV hull correction factors. This document is NOT an IEE Certificate or Class-approved EEXI technical file.</p>
          </div>
        </div>
      )}
