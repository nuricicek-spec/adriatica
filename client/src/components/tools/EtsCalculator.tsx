...
export function EtsCalculator() {
  ...
  const handleCalculate = () => {
    ...
    window.dispatchEvent(new CustomEvent("tool_compliance_update", { detail: { status: isHighRisk ? "non-compliant" : "compliant" } }));
  };
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      const fuelData = FUEL_TYPES.find(f => f.value === fuelType);
      await generateReportPdf(pdfRef, "Adriatica_ETS_Preliminary_Report.pdf",
        [
    ...
    <div className="bg-white border border-border/40 rounded-sm p-6 md:p-8 shadow-sm">
      ...
      <div className="space-y-6">
        <div className="p-4 bg-neutral-50 rounded-sm border border-border/20">
          <h3 className="text-sm font-bold text-[#0B3B5C] mb-3 uppercase tracking-wider">Operational & Market Data</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="number" value={annualFuel} onChange={e => setAnnualFuel(e.target.value)} placeholder="5000" className="w-full p-2 border rounded-sm text-base focus:border-primary outline-none" />
            </div>
            <div>
            ...
            <div>
              ...
              <input type="number" step="0.1" value={euaPrice} onChange={e => setEuaPrice(e.target.value)} placeholder="65" className="w-full p-2 border rounded-sm text-base focus:border-primary outline-none" />
      ...
      <button onClick={handleCalculate} className="w-full py-3 bg-primary text-white font-medium rounded-sm hover:bg-primary/90 transition mt-6">Estimate ETS Cost</button>
      ...
      {result !== null && (
        <div ref={pdfRef} className="mt-6 p-6 bg-white border-2 rounded-sm no-break">
          ...
          <div className={`p-4 border-2 rounded-sm mb-4 ${result.isHighRisk ? "border-red-500 bg-red-50" : "border-green-500 bg-green-50"}`}>
            <h4 className={`text-base font-bold mb-2 ${result.isHighRisk ? "text-red-800" : "text-green-800"}`}>ESTIMATED ANNUAL COST: €{result.estimatedCost}</h4>
            <div className="grid grid-cols-3 gap-4 text-xs mt-4 bg-white/50 p-3 rounded-sm no-break">
              <div><span className="text-gray-600 block">Total CO2 (MT)</span><p className="font-bold text-base text-[#0B3B5C]">{result.totalCo2}</p></div>
              <div><span className="text-gray-600 block">EUA Allowances</span><p className="font-bold text-base text-[#0B3B5C]">{result.requiredEua}</p></div>
              <div><span className="text-gray-600 block">Surrender Rate ({targetYear})</span><p className="font-bold text-base text-[#0B3B5C]">%{result.phaseIn}</p></div>
            </div>
          </div>
