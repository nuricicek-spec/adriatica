...
export function FueleuCalculator() {
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      await generateReportPdf(pdfRef, "Adriatica_FuelEU_Preliminary_Report.pdf",
        [
  ...
  return (
    <div className="bg-white border border-border/40 rounded-sm p-6 md:p-8 shadow-sm">
      ...
      <button onClick={handleCalculate} className="w-full py-3 bg-primary text-white font-medium rounded-sm hover:bg-primary/90 transition mt-6">Predict FuelEU Status</button>
      ...
      {result !== null && (
        <div ref={pdfRef} className="mt-6 p-6 bg-white border-2 rounded-sm no-break">
          ...
          <div className={`p-4 border-2 rounded-sm mb-4 ${result.isCompliant ? "border-green-500 bg-green-50" : "border-red-500 bg-red-50"}`}>
            <h4 className={`text-base font-bold mb-2 ${result.isCompliant ? "text-green-800" : "text-red-800"}`}>STATUS: {result.isCompliant ? "COMPLIANT" : "NON-COMPLIANT - PENALTY TRIGGERED"}</h4>
            <div className="grid grid-cols-3 gap-4 text-xs mt-4 bg-white/50 p-3 rounded-sm no-break">
              <div><span className="text-gray-600 block">Attained GHG (WTW)</span><p className="font-bold text-base text-[#0B3B5C]">{result.attainedGhg}</p></div>
              <div><span className="text-gray-600 block">Required Target ({targetYear})</span><p className="font-bold text-base text-[#0B3B5C]">{result.requiredGhg}</p></div>
              <div><span className="text-gray-600 block">Total Energy (GJ)</span><p className="font-bold text-base text-[#0B3B5C]">{result.totalEnergyGj}</p></div>
            </div>
            {!result.isCompliant && (
