...
export function FueleuCalculator() {
  const applicableFuels = useMemo(
    () => FUEL_TYPES.filter((f) => ["VLSFO", "HFO", "LNG", "MGO", "Methanol"].includes(f.value)),
    []
  );
  ...
  const [result, setResult] = useState<FueleuResult | null>(null);
  ...
  const handleCalculate = () => {
    ...
    const finalResult: FueleuResult = {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                type="number"
                value={annualFuel}
                onChange={(e) => setAnnualFuel(e.target.value)}
                placeholder="5000"
            ...
            <div className="md:col-span-2">
              ...
              <select
                ...
                className="w-full p-2 border rounded-sm bg-white text-base focus:border-primary outline-none"
              >
                {Object.keys(FUELEU_REDUCTION_FACTORS).map((y) => (
      ...
      {result !== null && (
        <div ref={pdfRef} className="mt-6 p-6 bg-white border-2 rounded-sm break-inside-avoid">
          ...
          <div className="bg-gray-50 p-3 rounded-sm border text-[10px] text-gray-600 leading-relaxed">
            ...
            <p>
              Uses fixed Well-to-Wake (WTW) factors from EC 2023/2543. Does not account for biofuel blending or RNFBO usage which would lower the attained intensity. Penalty calculated strictly via Regulation (EU) 2023/1804 Art. 14 formula using VLSFO equivalent baseline.
            </p>
          </div>
        </div>
      )}
