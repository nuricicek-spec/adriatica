    ...
    return (
      <div className="bg-white border border-border/40 rounded-sm p-6 md:p-8 shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <button onClick={() => setStep("quick")} className="p-6 border-2 border-primary/20 rounded-sm hover:border-primary hover:bg-primary/5 transition text-left group">
      ...
      <div className="bg-white border border-border/40 rounded-sm p-6 md:p-8 shadow-sm">
        ...
        <div className="space-y-6">
          <div className="p-4 bg-neutral-50 rounded-sm border border-border/20">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="number" value={quickLength} onChange={(e) => setQuickLength(e.target.value)} placeholder="50" className="w-full p-2 border rounded-sm text-base focus:border-primary outline-none" />
              </div>
              <div>
                <label className="block text-xs font-medium text-muted-foreground mb-1">Engine Type</label>
                <select value={quickEngineType} onChange={(e) => setQuickEngineType(e.target.value)} className="w-full p-2 border rounded-sm bg-white text-base focus:border-primary outline-none">
          ...
          <div className="p-4 bg-blue-50 border border-blue-200 rounded-sm">
            ...
            <p className="text-[10px] text-blue-600 mb-2">* DWT, MCR, Vref, and SFC estimated from length. Annual fuel & distance estimated from DWT.</p>
        ...
        <button onClick={handleQuickContinue} className="w-full py-3 bg-primary text-white font-medium rounded-sm hover:bg-primary/90 transition mt-6 flex items-center justify-center gap-2">
          ...
          <div className="p-4 bg-neutral-50 rounded-sm border border-border/20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <select value={profile.vesselType} onChange={(e) => setField("vesselType", e.target.value)} className="w-full p-2 border rounded-sm bg-white text-base focus:border-primary outline-none">
              ...
              <div>
                ...
                <input type="number" value={profile.dwt || ""} onChange={(e) => setField("dwt", parseFloat(e.target.value) || 0)} placeholder="50000" className="w-full p-2 border rounded-sm text-base focus:border-primary outline-none" />
              </div>
              <div className="md:col-span-2">
                <label className="block text-xs font-medium text-muted-foreground mb-1">Target Year</label>
                <input type="number" value={profile.targetYear || ""} onChange={(e) => setField("targetYear", parseInt(e.target.value) || 2026)} placeholder="2026" className="w-full p-2 border rounded-sm text-base focus:border-primary outline-none" />
              </div>
            </div>
          </div>
          <div className="p-4 bg-neutral-50 rounded-sm border border-border/20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input type="number" value={profile.meMcr || ""} onChange={(e) => setField("meMcr", parseFloat(e.target.value) || 0)} placeholder="8000" className="w-full p-2 border rounded-sm text-base focus:border-primary outline-none" />
              </div>
              <div>
                <label className="block text-xs font-medium text-muted-foreground mb-1">Fuel Type</label>
                <select value={profile.meFuel} onChange={(e) => setField("meFuel", e.target.value)} className="w-full p-2 border rounded-sm bg-white text-base focus:border-primary outline-none">
                  {FUEL_TYPES.map((f) => (<option key={f.value} value={f.value}>{f.label}</option>))}
                </select>
              </div>
              <div>
                <label className="block text-xs font-medium text-muted-foreground mb-1">SFC (g/kWh)</label>
                <input type="number" value={profile.meSfc || ""} onChange={(e) => setField("meSfc", parseFloat(e.target.value) || 0)} placeholder="190" className="w-full p-2 border rounded-sm text-base focus:border-primary outline-none" />
              </div>
              <div>
                <label className="block text-xs font-medium text-muted-foreground mb-1">Vref (Knots)</label>
                <input type="number" step="0.1" value={profile.vref || ""} onChange={(e) => setField("vref", parseFloat(e.target.value) || 0)} placeholder="14.5" className="w-full p-2 border rounded-sm text-base focus:border-primary outline-none" />
              </div>
            </div>
          </div>
          <div className="p-4 bg-neutral-50 rounded-sm border border-border/20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input type="number" value={profile.auxPower || ""} onChange={(e) => setField("auxPower", parseFloat(e.target.value) || 0)} placeholder="500" className="w-full p-2 border rounded-sm text-base focus:border-primary outline-none" />
              </div>
              <div>
                <label className="block text-xs font-medium text-muted-foreground mb-1">Aux SFC (g/kWh)</label>
                <input type="number" value={profile.auxSfc || ""} onChange={(e) => setField("auxSfc", parseFloat(e.target.value) || 0)} placeholder="215" className="w-full p-2 border rounded-sm text-base focus:border-primary outline-none" />
              </div>
              <div className="flex items-center gap-2 mt-2">
                <input type="checkbox" id="ptoCheck" checked={profile.hasPto} onChange={(e) => setField("hasPto", e.target.checked)} className="rounded border-border text-primary" />
              ...
              {profile.hasPto && (
                <>
                  <div>
                    ...
                    <input type="number" value={profile.ptoPower || ""} onChange={(e) => setField("ptoPower", parseFloat(e.target.value) || 0)} placeholder="1000" className="w-full p-2 border rounded-sm text-base focus:border-primary outline-none" />
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-muted-foreground mb-1">PTO Efficiency</label>
                    <input type="number" step="0.1" value={profile.ptoEff || ""} onChange={(e) => setField("ptoEff", parseFloat(e.target.value) || 0)} placeholder="1.0" className="w-full p-2 border rounded-sm text-base focus:border-primary outline-none" />
                  </div>
                </>
              )}
            </div>
          ...
          <div className="p-4 bg-neutral-50 rounded-sm border border-border/20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input type="number" step="0.1" value={profile.annualFuel || ""} onChange={(e) => setField("annualFuel", parseFloat(e.target.value) || 0)} placeholder="5000" className="w-full p-2 border rounded-sm text-base focus:border-primary outline-none" />
              </div>
              <div>
                <label className="block text-xs font-medium text-muted-foreground mb-1">Annual Distance (NM)</label>
                <input type="number" value={profile.annualDistance || ""} onChange={(e) => setField("annualDistance", parseFloat(e.target.value) || 0)} placeholder="15000" className="w-full p-2 border rounded-sm text-base focus:border-primary outline-none" />
              </div>
              <div>
                <label className="block text-xs font-medium text-muted-foreground mb-1">EUA Price (€/ton CO₂)</label>
                <input type="number" step="0.1" value={euaPrice} onChange={(e) => setEuaPrice(parseFloat(e.target.value) || 65)} placeholder="65" className="w-full p-2 border rounded-sm text-base focus:border-primary outline-none" />
  ...
  if (step === "scenarios") {
    ...
    return (
      <div className="bg-white border border-border/40 rounded-sm p-6 md:p-8 shadow-sm">
          ...
          {SCENARIOS.map((scenario) => {
            ...
            return (
              <button
                ...
                } ${isCurrent ? "opacity-75 cursor-default" : ""} ${!isSelected && !canAddMore && !isCurrent ? "opacity-50 cursor-not-allowed" : ""}`}
              >
                ...
                <div className="flex-1">
                  <div className="flex justify-between items-start">
                    ...
                    {isSelected && !isCurrent && <span className="text-[10px] bg-primary text-white px-1.5 py-0.5 rounded-sm">Selected</span>}
                  </div>
        ...
        <button onClick={() => { runCalculations(); setStep("results"); }} disabled={selectedScenarios.length === 0} className="w-full py-3 bg-primary text-white font-medium rounded-sm hover:bg-primary/90 transition flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
      ...
      <div className="bg-white border border-border/40 rounded-sm p-6 md:p-8 shadow-sm">
          ...
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="bg-neutral-50 border-b-2 border-border">
                ...
                {results.map((r) => (
                  <th key={r.scenarioId} className={`p-3 text-center font-bold ${r.scenarioId === "current" ? "text-[#0B3B5C]" : "text-primary"}`}>
                    {r.label}
                  </th>
                ))}
              </tr>
            ...
            <tbody className="divide-y divide-border/20">
              ...
              <tr className="bg-neutral-50/50">
                ...
                {results.map((r) => (
                  <td key={r.scenarioId} className="p-3 text-center text-muted-foreground">{r.eexi.required.toFixed(2)}</td>
                ))}
              </tr>
              <tr>
                ...
                {results.map((r) => (
                  <td key={r.scenarioId} className={`p-3 text-center font-bold ${
                    r.cii.rating === "A" || r.cii.rating === "B" ? "text-green-600" : r.cii.rating === "C" ? "text-yellow-600" : "text-red-600"
                  }`}>{r.cii.rating}</td>
                ))}
              </tr>
              <tr className="bg-neutral-50/50">
              ...
              <tr>
                ...
                {results.map((r) => (
                  <td key={r.scenarioId} className={`p-3 text-center ${r.fueleu.penalty > 0 ? "text-red-600 font-bold" : "text-green-600"}`}>
        ...
        <div className="mt-6 p-4 bg-primary/5 border border-primary/20 rounded-sm">
          ...
          <p className="text-sm text-muted-foreground">
            ...
            {complianceStatus === "non-compliant" ? " Non-compliant scenarios detected. See red indicators above." : " All scenarios meet current regulatory thresholds."}
          </p>
        </div>
        <div className="mt-6 flex gap-3">
          <button onClick={() => setStep("mode")} className="flex-1 py-2.5 border border-primary text-primary font-medium rounded-sm hover:bg-primary/5 transition text-sm">New Comparison</button>
          <button onClick={() => setStep("scenarios")} className="flex-1 py-2.5 bg-primary text-white font-medium rounded-sm hover:bg-primary/90 transition text-sm">Modify Scenarios</button>
        </div>
      </div>
    );
  }
