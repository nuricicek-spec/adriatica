  ...
  const handleCalculate = () => {
    ...
    window.dispatchEvent(
      new CustomEvent("tool_compliance_update", {
        detail: { status: res.isCompliant ? "compliant" : "non-compliant" },
      })
    );
  };
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      await generateReportPdf(
        pdfRef,
        "Adriatica_ShaPoLi_Preliminary_Report.pdf",
        [
          ...
          <div className="flex flex-col gap-3">
            <label className="flex items-center gap-3 cursor-pointer p-3 border rounded-sm hover:bg-white transition">
              <input
                ...
                onChange={() => setIsOverridable(true)}
            ...
            <label className="flex items-center gap-3 cursor-pointer p-3 border rounded-sm hover:bg-white transition">
              <input
                ...
                name="shapoliType"
                checked={!isOverridable}
                onChange={() => setIsOverridable(false)}
                className="text-primary"
              />
              <div>
          ...
          {showGuidance && (
            <div className="mt-3 p-3 bg-blue-50 border border-blue-200 rounded-sm text-xs text-blue-800">
              <p className="font-bold mb-1">
                {isOverridable ? SHAPOLI_GUIDANCE.overridable.title : SHAPOLI_GUIDANCE.nonOverridable.title}
              </p>
              <p className="mb-2">
                {isOverridable ? SHAPOLI_GUIDANCE.overridable.description : SHAPOLI_GUIDANCE.nonOverridable.description}
              </p>
              <ul className="list-disc list-inside space-y-1">
                {(isOverridable ? SHAPOLI_GUIDANCE.overridable.requirements : SHAPOLI_GUIDANCE.nonOverridable.requirements).map(
                  (req, i) => (
                    <li key={i}>{req}</li>
                  )
                )}
      ...
      {result !== null && (
        <div ref={pdfRef} className="mt-6 p-6 bg-white border-2 rounded-sm break-inside-avoid">
          ...
          <div className={`p-4 border-2 rounded-sm mb-4 ${result.isCompliant ? "border-green-500 bg-green-50" : "border-red-500 bg-red-50"}`}>
            ...
            <div className="grid grid-cols-2 gap-4 text-sm mt-4 bg-white/50 p-3 rounded-sm break-inside-avoid">
              <div>
                <span className="text-xs text-gray-600">Measured Shaft Power</span>
                <p className="font-bold text-xl text-[#0B3B5C]">
                  {result.measuredShaftPower} <span className="text-sm font-normal">kW</span>
                </p>
              </div>
              <div>
                <span className="text-xs text-gray-600">ShaPoLi Limit</span>
                <p className="font-bold text-xl text-[#0B3B5C]">
                  {result.limitValue.toFixed(0)} <span className="text-sm font-normal">kW</span>
                </p>
              </div>
            </div>
            {!result.isCompliant && (
              <div className="mt-4 pt-3 border-t border-red-300 bg-white/50 p-3 rounded-sm break-inside-avoid">
                <h5 className="font-bold text-sm text-red-800 mb-1">OVERRIDE CAPACITY</h5>
                <p className="font-mono font-bold text-lg text-[#0B3B5C]">
                  {result.maxOverridePower.toFixed(0)} kW <span className="text-xs font-normal text-gray-600">(110% of limit)</span>
                </p>
              </div>
            )}
          </div>
          <div className="bg-gray-50 p-3 rounded-sm border text-[10px] text-gray-600 leading-relaxed">
            ...
            <p>
              ShaPoLi limit calculated as percentage of MCR based on vessel size: {result.limitValue < result.measuredShaftPower * 0.8 ? "75%" : "80-85%"}. Overridable systems allow 110% transient power for safety per MEPC.350(78). Log accuracy requirement: {result.requiredLogAccuracy}. This is NOT a Class-approved ShaPoLi technical file.
            </p>
          </div>
        </div>
      )}
