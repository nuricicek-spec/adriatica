        ...
        <div className="p-4 bg-neutral-50 rounded-sm border border-border/20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="number" value={shaftPower} onChange={e => setShaftPower(e.target.value)} placeholder="7200" className="w-full p-2 border rounded-sm text-sm focus:border-primary outline-none" />
            </div>
          </div>
        </div>
      ...
      {result !== null && (
        <div ref={pdfRef} className="mt-6 p-6 bg-white border-2 rounded-sm no-break">
          ...
          <div className={`p-4 border-2 rounded-sm mb-4 ${result.isCompliant ? "border-green-500 bg-green-50" : "border-red-500 bg-red-50"}`}>
            ...
            <div className="grid grid-cols-2 gap-4 text-sm mt-4 bg-white/50 p-3 rounded-sm no-break">
              <div>
                ...
                <p className="font-bold text-xl text-[#0B3B5C]">{result.measuredShaftPower} <span className="text-sm font-normal">kW</span></p>
              </div>
              <div>
                <span className="text-xs text-gray-600">ShaPoLi Limit</span>
                <p className="font-bold text-xl text-[#0B3B5C]">{result.limitValue.toFixed(0)} <span className="text-sm font-normal">kW</span></p>
              </div>
            </div>
            {!result.isCompliant && (
          ...
          <div className="bg-gray-50 p-3 rounded-sm border text-[10px] text-gray-600 leading-relaxed">
            ...
            <p>
              ShaPoLi limit calculated as percentage of MCR based on vessel size: {result.limitValue < result.measuredShaftPower * 0.8 ? "75%" : "80-85%"}.
              Overridable systems allow 110% transient power for safety per MEPC.350(78).
              Log accuracy requirement: {result.requiredLogAccuracy}.
              This is NOT a Class-approved ShaPoLi technical file.
            </p>
          </div>
        </div>
      )}
