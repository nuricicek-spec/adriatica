...
export function BwtsCalculator() {
  ...
  const handleCalculate = () => {
    ...
    window.dispatchEvent(
      new CustomEvent("tool_compliance_update", { detail: { status: "compliant" } })
    );
  };
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      const marginLabel =
        safetyMargin === "1.10" ? "10% (Standard Clean Water)"
  ...
  return (
    <div className="bg-white border border-border/40 rounded-sm p-6 md:p-8 shadow-sm">
      ...
      <p className="text-xs text-muted-foreground mb-6">
        Retrofit sizing based on peak ballast pump discharge rates and system safety margins.
      </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="md:col-span-2">
              <label className="block text-xs font-medium text-muted-foreground mb-1">
                Largest Ballast Pump Design Capacity (m³/h)
              </label>
              <input
                type="number"
                value={pumpDesignCap}
                onChange={(e) => setPumpDesignCap(e.target.value)}
                placeholder="e.g., 250"
            ...
            <div className="md:col-span-2">
              ...
              <p className="text-[10px] text-muted-foreground mt-1">
                * Select the maximum number of pumps that can discharge into the ballast system at the same time. If pumps are never run simultaneously, select 1. Over-selecting leads to unnecessary oversizing of the BWTS unit.
              </p>
            </div>
          </div>
        </div>
      ...
      {result !== null && (
        <div ref={pdfRef} className="mt-6 p-6 bg-white border-2 border-primary rounded-sm break-inside-avoid">
          ...
          <div className="p-4 bg-primary/5 border-2 border-primary rounded-sm mb-4 break-inside-avoid">
            <div className="flex justify-between items-end mb-2">
              <h4 className="text-sm font-bold text-[#0B3B5C]">Minimum Required BWTS Capacity</h4>
              <p className="text-3xl font-display font-bold text-primary">
                {result} <span className="text-sm font-normal">m³/h</span>
              </p>
            </div>
            <p className="text-[10px] text-gray-600">
              Calculated with {(parseFloat(safetyMargin) - 1) * 100}% system margin — based on {pumpCount} pump(s) operating simultaneously
            </p>
          </div>
          <div className="bg-gray-50 p-3 rounded-sm border text-xs text-gray-700">
            ...
            <p>
              When selecting a BWTS vendor, ensure the system's rated capacity (at the worst-case filtrate differential pressure) meets or exceeds this calculated value. Undersized systems lead to bypass scenarios during peak ballast operations, resulting in PSC deficiencies under the BWM Convention.
            </p>
          </div>
          <div className="mt-4 bg-gray-50 p-3 rounded-sm border text-[10px] text-gray-600 leading-relaxed">
            ...
            <p>
              BWTS capacity sized on peak simultaneous ballast pump discharge flow multiplied by a user-defined safety factor. "Simultaneous" refers to pumps that can discharge into the ballast system at the same time during worst-case operations. Does not account for filter backwash volumes or OEM-specific self-cleaning cycles.
            </p>
          </div>
        </div>
      )}
