...
const bm25Docs = allContent.map(item => ({
  id: item.slug,
  text: [item.title, item.description || '', ...item.tags.map(t => t.name)].join(' '),
}));

export const bm25 = initBM25(bm25Docs, { k1: 1.2, b: 0.75 });